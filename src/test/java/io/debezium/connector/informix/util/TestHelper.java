...
public class TestHelper {
    ...
    public static Configuration.Builder defaultConfig() {
        ...
        return Configuration.copy(defaultJdbcConfig().build().map(key -> ConfigurationNames.DATABASE_CONFIG_PREFIX + key))
                .with(CommonConnectorConfig.EXECUTOR_SHUTDOWN_TIMEOUT_MS, 46_983)
                .with(AsyncEngineConfig.TASK_MANAGEMENT_TIMEOUT_MS, 199_034)
                .with(CommonConnectorConfig.TOPIC_PREFIX, TEST_DATABASE)
                .with(CommonConnectorConfig.DRIVER_CONFIG_PREFIX + IFX_LOCK_MODE_WAIT, 13)
